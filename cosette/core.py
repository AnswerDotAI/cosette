...
# %% auto 0
__all__ = ['empty', 'mk_msg', 'mk_msgs', 'o1_models', 'models', 'models_azure', 'find_block', 'contents', 'usage', 'wrap_latex',
           'Client', 'get_stream', 'mk_openai_func', 'mk_tool_choice', 'call_func', 'mk_toolres', 'mock_tooluse',
           'Chat']
...
# %% ../00_core.ipynb 8
models = *o1_models, 'gpt-4o', 'gpt-4-turbo', 'gpt-4', 'gpt-4-32k', 'gpt-3.5-turbo', 'gpt-3.5-turbo-instruct'
...
# %% ../00_core.ipynb 37
class Client:
    def __init__(self, model, cli=None, text_only=False):
        "Basic LLM messages client."
        self.model,self.use,self.text_only = model,usage(0,0),text_only
...
def _o1_checks(
    ...
    **kwargs
    ...
    "Check that o1 models support the proposed call."
    assert not bool(sp), "System prompts are not supported by o1 models."
    assert not stream, "Streaming is not supported by o1 models."
    assert 'tools' not in kwargs, "Tool use is not supported by o1 models."
    msgs_have_imgs = any(c['type'] == 'image_url' for msg in msgs if isinstance(msg, dict) and isinstance(msg.get('content'), list) for c in msg['content'])
    assert not msgs_have_imgs, "Images are not supported by o1 models."   
...
def __call__(self:Client,
             ...
             **kwargs):
    "Make a call to LLM."
    if self.model in o1_models: _o1_checks(msgs, sp, stream, **kwargs)
    if stream: kwargs['stream_options'] = {"include_usage": True}
    if sp: msgs = [mk_msg(sp, 'system', text_only=self.text_only)] + list(msgs)
    r = self.c.create(
...
def __call__(self:Chat,
    ...
    if pr: self.h.append(mk_msg(pr, text_only=self.c.text_only))
